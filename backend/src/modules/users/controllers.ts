...
export class UserController {
  static async getChildProfile(req: Request, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({
          success: false,
      ...
      // Find child profile associated with this user
      ...
      const message = error instanceof Error ? error.message : 'Failed to get child profile';
  ...
  static async createChild(req: Request, res: Response): Promise<void> {
    try {
      logger.info('Create child request received', {
        user: req.user,
        body: req.body,
        headers: req.headers
      });
