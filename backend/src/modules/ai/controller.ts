...
export class AIController {
  static async askQuestion(req: Request, res: Response): Promise<void> {
    try {
      const { question, subject, gradeLevel, language, childId } = req.body;

      if (!question || typeof question !== 'string' || question.trim().length < 2) {
        res.status(400).json({ success: false, error: 'Invalid question' });
        return;
      }
      ...
      // Extract just the explanation as the answer
      ...
      if (req.user?.id) {
        try {
          ...
          logger.warn('Failed to log learning activity', logErr);
        }
      }
      ...
      logger.error('AI ask question error', err);
